...
# Prepare data
singularity run ${PIPE_PATH}gantry_notifications.simg -m "Downloading ${SCAN_DATE}" -c ${CHANNEL}
...
# Run pipeline workflow #1
singularity run ${PIPE_PATH}gantry_notifications.simg -m "Processing ${SCAN_DATE}" -c ${CHANNEL}
./entrypoint_p1.sh
singularity run ${PIPE_PATH}3d_sequential_align.simg -i preprocessing_out/ -o sequential_alignment_out/

# Run pipeline workflow #2
#./entrypoint_p2.sh

# Compress outputs
singularity run ${PIPE_PATH}gantry_notifications.simg -m "Compressing ${SCAN_DATE} outputs" -c ${CHANNEL}
...
#Upload outputs
singularity run ${PIPE_PATH}gantry_notifications.simg -m "Finished compressing ${SCAN_DATE}. Now uploading to CyVerse." -c ${CHANNEL}
ssh filexfer 'cd' "${PIPE_PATH}" '&& ./upload.sh' ${SCAN_DATE} ${PIPE_PATH} '&& exit'

#Cancel workers and clean the file system.
singularity run ${PIPE_PATH}gantry_notifications.simg -m "Upload complete. See ouputs at /iplant/home/shared/phytooracle/season_10_lettuce_yr_2020/level_1/scanner3DTop/${SCAN_DATE}" -c ${CHANNEL}
