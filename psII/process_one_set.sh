...
singularity run -B $(pwd):/mnt --pwd /mnt ${SIMG_PATH}bin2tif_psii.simg -m ${METADATA} -o ${WORKING_SPACE} ${HPC_PATH}${RAW_DATA_PATH}*.bin
...
singularity run -B $(pwd):/mnt --pwd /mnt ${SIMG_PATH}plotclip_shp.simg -sen ${SENSOR} -o ${PLOTCLIP_DIR} -shp ${GEOJ} ${TIF_RESIZE}*.tif
